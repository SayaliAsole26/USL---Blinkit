...
def test_enqueue_falls_back_to_sync_when_celery_unavailable(async_settings):
    with patch("app.worker.process_intent_task.delay", side_effect=ConnectionError("redis down")):
        with patch("app.services.intent_queue._process_intent_sync") as sync_mock:
            enqueue_intent_processing(TEST_ITEM_ID, TEST_USER_ID, trigger="created", settings=async_settings)
